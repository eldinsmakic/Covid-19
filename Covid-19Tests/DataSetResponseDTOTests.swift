...
class DataSetResponseDTOTests: XCTestCase {
    ...
    func testConvertion2() {
          let jsonDecoder  = JSONDecoder()
          jsonDecoder.keyDecodingStrategy = .convertFromSnakeCase

          do {
            let data = try jsonDecoder.decode(DataSetResponseDTO.self, from: fakeJson.data(using: .utf8)!)
              print(data)
              XCTAssertTrue(true)
